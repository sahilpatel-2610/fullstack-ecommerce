...
router.post(`/upload`, upload.array("images"), async (req, res) => {
    ...
    try {
        ...
        console.error("HomeSideBanner Upload Error:", error);
...
router.get(`/`, async (req, res) => {
    try {
        ...
        if (req.query.page !== undefined && req.query.perPage !== undefined) {
            const totalPosts = await HomeSideBanner.countDocuments();
            totalPages = Math.ceil(totalPosts / perPage);

            bannerList = await HomeSideBanner.find()
                .skip((page - 1) * perPage)
                .limit(perPage)
                .exec();
        } else {
...
router.get('/:id', async (req, res) => {
    try {
        const banner = await HomeSideBanner.findById(req.params.id);
        if (!banner) {
...
router.post('/create', async (req, res) => {
    try {
        ...
        let newEntry = new HomeSideBanner({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const banner = await HomeSideBanner.findByIdAndUpdate(
            req.params.id,
            {
                images: req.body.images,
                bannerTitle: req.body.bannerTitle || req.body.name || "",
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await HomeSideBanner.findByIdAndDelete(req.params.id);
