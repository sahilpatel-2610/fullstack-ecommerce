...
router.get(`/`, async (req, res) => {
    try {
        ...
        if (req.query.page !== undefined && req.query.perPage !== undefined) {
            const totalPosts = await SidebarBanner.countDocuments();
            totalPages = Math.ceil(totalPosts / perPage);

            bannerList = await SidebarBanner.find()
                .skip((page - 1) * perPage)
                .limit(perPage)
                .exec();
        } else {
...
router.get('/:id', async (req, res) => {
    try {
        const banner = await SidebarBanner.findById(req.params.id);
        if (!banner) {
...
router.post('/create', async (req, res) => {
    try {
        ...
        let newEntry = new SidebarBanner({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const banner = await SidebarBanner.findByIdAndUpdate(
            req.params.id,
            {
                images: req.body.images,
                bannerTitle: req.body.bannerTitle || req.body.name || "",
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await SidebarBanner.findByIdAndDelete(req.params.id);
